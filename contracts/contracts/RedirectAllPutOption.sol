...
contract RedirectAllPutOption is SuperAppBase {
    ...
    uint8 public _underlyingDecimals = 18;

    //asset that can be used to purchase _underlyingAsset
    ERC20 public _purchasingAsset;
    //string price as purchasing asset ERC20
    ...
    //decimal of asset that can be used to purchase _underlyingAsset
    uint256 public _purchasingDecimals;
    ...
    function _activateOption() internal {
        ...
        _dai.transferFrom(_receiver, address(this), _underlyingAmount);
    ...
    function exerciseOption() external {
        require(
            _purchasingAsset.allowance(msg.sender, address(this)) >=
                uint256(_strikePrice),
            "must call approve first"
        );
        ...
        if (block.timestamp >= _expirationDate) {
            ...
            //need to reconsider the math here

            if (_priceFeedDecimals != _underlyingDecimals) {
                int256 _adjustedDecimals = int256(
                    uint256(_underlyingDecimals - _priceFeedDecimals)
                );
                ...
                currentPrice = int256(
                    uint256(currentPrice) * (10**uint256(_adjustedDecimals))
                );
            }
